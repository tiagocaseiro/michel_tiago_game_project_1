...
GameManager::GameManager(const FormationRules& formationRules, const GameplayRules& gameplayRules,
    ...
    : mBoard(mPlayer1, mPlayer2, initialSetup),
      ...
      mActionManager(mBoard, mTurnManager),
...
{
    constexpr bool advancesTurn = false;
    auto actionParams = std::make_unique<DrawCardsActionParams>(mBoard.mTroopDeck, player,
                                                                initialSetup.mInitialCardsPerPlayer, advancesTurn);
